...
export const resolvers = {
  Query: {
    ...
    getAllSurat: async () => await Surat.find().sort({ tanggalPengajuan: -1 }),
    getSuratByCitizen: async (_, { citizenId }) => await Surat.find({ citizenId }).sort({ tanggalPengajuan: -1 })
    // ----------------------------------
  },
  ...
  Mutation: {
    ...
    ajukanSurat: async (_, { citizenId, jenisSurat, keperluan }) => {
      try {
        ...
        return await suratBaru.save();
      } catch (error) {
        throw new Error("Gagal mengajukan surat: " + error.message);
      }
    },
    updateStatusSurat: async (_, { id, status }) => {
      try {
        return await Surat.findByIdAndUpdate(id, { $set: { status } }, { new: true });
      } catch (error) {
        throw new Error("Gagal update status surat: " + error.message);
      }
    },
    deleteSurat: async (_, { id }) => {
